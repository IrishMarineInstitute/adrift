...
class OpenDriftSimulation(PhysicsMethods, Timeable, Configurable):
    ...
    def closest_ocean_points(self, lon, lat):
        ...
        if not 'land_binary_mask' in self.env.priority_list:
            ...
            land_reader_covers = False
            land_reader_counter = -1
            while not land_reader_covers:
                land_reader_counter += 1
                land_reader_name = self.env.priority_list['land_binary_mask'][land_reader_counter]
                land_reader = self.env.readers[land_reader_name]
                latgrid_covered = land_reader.covers_positions(longrid, latgrid)[0]
                if latgrid_covered.shape != (0,):
                    land_reader_covers = True           
            o = self

        if land_reader_name == 'global_landmask':
            land_reader.start_time = datetime(1970, 1, 1)
            land_reader.end_time = datetime(2100, 1, 1)
